...
# 使用 GridSearchCV 进行参数调优和交叉验证
grid_search = GridSearchCV(estimator=xgb_model, param_grid=param_grid, cv=kfolds, scoring='neg_mean_squared_error', verbose=1)
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_XGBoost.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_XGBoost.csv')
#保存训练集数据
