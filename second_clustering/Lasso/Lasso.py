...
def gen_models(model_name):
    if model_name == 'LR':
        ...
        return make_pipeline(RobustScaler(), LassoCV(max_iter=10000000, alphas=alphas2, random_state=42, cv=kfolds))
...
y_pre_pre,model = train_test(X_train, X_test, y_train, y_test, 'Lasso')
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_Lasso.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_Lasso.csv')
