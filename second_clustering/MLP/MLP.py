...
#     return pre_y,model
#
# y_pre_pre,model = train_test(X_train, X_test, y_train, y_test, 'MLP')

# 定义 MLPRegressor 的超参数搜索网格
param_grid = {
    'mlpregressor__hidden_layer_sizes': [(50, 50), (100, 100), (200, 200)],  # 隐藏层大小
    'mlpregressor__activation': ['relu', 'tanh'],  # 激活函数
    'mlpregressor__solver': ['adam', 'lbfgs'],  # 优化算法
    'mlpregressor__alpha': [0.0001, 0.001, 0.01],  # L2 正则化强度
    'mlpregressor__learning_rate': ['constant', 'adaptive'],  # 学习率策略
    'mlpregressor__max_iter': [1000]  # 最大迭代次数
}

# 创建 MLPRegressor 模型流水线，包含 RobustScaler 和 MLPRegressor
model = make_pipeline(
    RobustScaler(),
...
grid_search = GridSearchCV(model, param_grid, cv=kfolds, scoring='neg_mean_squared_error', n_jobs=-1)
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_MLP.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_MLP.csv')
#保存训练集数据
