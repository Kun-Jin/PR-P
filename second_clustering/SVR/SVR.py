...
data=data_o.drop(['time','hour','Arrival_passenger','wandan','first_1h_confirm',
...
#     return pre_y,model
#
# y_pre_pre,model = train_test(X_train, X_test, y_train, y_test, 'SVR')
...
param_grid = {
    ...
    'C': [0.1, 1, 10],  # 正则化参数
    'epsilon': [0.01, 0.1, 0.2],  # 对误差的容忍度
    'degree': [3, 4, 5]  # 多项式核函数的度数（仅对 poly 核函数有效）
}

# 使用 GridSearchCV 进行超参数调优和交叉验证
grid_search = GridSearchCV(estimator=svr, param_grid=param_grid, cv=kfolds, scoring='neg_mean_squared_error', n_jobs=-1)
...
#误差
mse = np.sum((data.iloc[len(data)-num:len(data),0].values - inverse_data[:,0]) ** 2) / len(data.iloc[len(data)-num:len(data),4].values)
rmse = sqrt(mse)
mae = np.sum(np.absolute(data.iloc[len(data)-num:len(data),0].values - inverse_data[:,0])) / num
mape = np.sum(np.absolute(data.iloc[len(data)-num:len(data),0].values - inverse_data[:,0])/data.iloc[len(data)-num:len(data),4].values) / num
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_SVR.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_SVR.csv')
#保存训练集数据
