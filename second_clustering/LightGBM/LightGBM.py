...
#     return pre_y,model
#
# y_pre_pre,model = train_test(X_train, X_test, y_train, y_test, 'LightGBM')
...
# 定义超参数网格
param_grid = {
    'num_leaves': [31, 50],  # 树的叶子节点数
    'max_depth': [5, 10, -1],  # 树的最大深度
    'learning_rate': [0.05, 0.1],  # 学习率
    'n_estimators': [50, 100],  # 基学习器的数量
}

# 使用 GridSearchCV 进行超参数调优和交叉验证
grid_search = GridSearchCV(estimator=model, param_grid=param_grid, cv=kfolds, scoring='neg_mean_squared_error', n_jobs=-1)
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_LightGBM.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_LightGBM.csv')
#保存训练集数据
