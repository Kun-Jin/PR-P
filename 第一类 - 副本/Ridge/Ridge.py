...
y_pre_pre,model = train_test(X_train, X_test, y_train, y_test, 'Ridge')
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_Ridge.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_Ridge.csv')
#保存训练集数据
