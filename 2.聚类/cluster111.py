...
data = pd.read_csv(r'./需要聚类.csv',encoding='gb18030')
data_cluster=pd.DataFrame()
data_cluster['全天均值']=np.mean(data.iloc[:,1:],axis=1)
data_cluster['全天偏度']=data.iloc[:,1:].skew(axis=1)
data_cluster['20-22均值']=np.mean(data.iloc[:,21:23],axis=1)
data_cluster['22']=data.iloc[:,23]
data_cluster['23']=data.iloc[:,24]
data_cluster['20-24峰值']=data[['1','20','21','22','23']].max(axis=1)
data_cluster['0-5峰值']=data[['0','1','2','3','4','5']].max(axis=1)
data_cluster['昨日确诊']=data["昨日确诊"]
data_cluster = data_cluster * [0.15, 0.1, 0.1, 0.1, 0.1, 0.15, 0.15, 0.15]
#聚类
from pylab import *
import pandas as pd
import numpy as np
