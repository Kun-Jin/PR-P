...
#     return pre_y,model
#
# y_pre_pre,model = train_test(X_train, X_test, y_train, y_test, 'GradientBoosting')
...
# 使用 GridSearchCV 进行 K 折交叉验证（cv=5）
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_GradientBoosting.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_GradientBoosting.csv')
#保存训练集数据
