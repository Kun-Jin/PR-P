...
y_pre_pre,model = train_test(X_train, X_test, y_train, y_test, 'LR')
...
pd.DataFrame(inverse_data[:,0]).to_csv('./predict_LR.csv')
data.iloc[len(data)-num:len(data),0].to_csv('./real.csv')
pd.DataFrame(inverse_data[:,0]-data.iloc[len(data)-num:len(data),0].values).to_csv('./error_LR.csv')
